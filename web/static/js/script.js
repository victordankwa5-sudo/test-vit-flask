...
if (window.location.pathname.includes('/view-product') || window.location.pathname.includes('/profile') || window.location.pathname.includes('/edit-personal-info') || window.location.pathname.includes('/search')) {
  ...
  btn_container.innerHTML = `<button class="back-btn" style="font-size: 1rem; background: none; border: none; font-weight: 800; width: 20px;"><i class="fas fa-back"></i> < </button>`;
...
if (window.location.pathname.includes('/products') || window.location.pathname.includes('/search')) {
  ...
  filterEl.addEventListener('input', event => {
    productCat.forEach((cat) => {
      ...
      if (searchData == 'all') {
        ...
        cat.parentNode.parentNode.classList.toggle('hide', searchQuery != searchData);
      }
...
if (window.location.pathname === '/' || window.location.pathname.includes('/products') || window.location.pathname.includes('/search')) {
  const searchEl = document.getElementById('search-box')
  searchEl.style.display = 'flex'

}

if (window.location.pathname.includes('/profile') || window.location.pathname.includes('/edit-personal-info')) {
...
function change_image(img_src) {
  ...
  img_btn.forEach((btn) => {
    if (main_image.getAttribute('src') !== btn.children[0].getAttribute('src')) {
  ...
  .forEach((btn) => {
    btn.addEventListener("click", (event) => {
      ...
      fetch("/add-to-cart")
        ...
        .then((json) => {
          cart_quantity.forEach((qt) => {qt.textContent = json.new_cart_quantity});
...
function changeTown() {
  ...
  if (region.value == "nothern region") {
