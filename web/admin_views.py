...
from.models import Order, User, Product, Brands
...
def admin_index():
    if current_user.id == 1:
        ...
        return render_template('admin-dashboard.html', user=current_user, total_products=total_products, total_orders=total_orders, total_customers=total_customers, orders=orders)
    else:
...
def admin_products():
    ...
    if id == 1:
        ...
        return render_template('admin-product.html', user=current_user, products=products, total_orders=total_orders)
    else:
        flash("You are not an admin!", "error")
        return redirect(url_for("views.index"))
...
def orders():
    if current_user.id == 1:
        ...
        return render_template('admin-orders.html', user=current_user, orders=orders, total_orders=total_orders)
    else:
        flash('You are not an admin!', 'error')
        return redirect(url_for('views.index'))
...
def customers():
    if current_user.id == 1:
        ...
        return render_template('admin-customers.html', user=current_user, customers=customers, total_orders=total_orders)
    else:
        flash('You are not an admin!', 'error')
        return redirect(url_for('views.index'))
...
def admin_brands():
    if current_user.id == 1:
        ...
        return render_template('admin-brands.html', user=current_user, brands=brands, total_orders=total_orders)
    else:
        flash('You are not an admin!', 'error')