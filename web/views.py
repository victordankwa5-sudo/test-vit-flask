...
from .models import Product, Cart, Payment, Order, User
from . import db
import json
...
def index():
    ...
    return render_template("index.html", user=current_user, products=products, cart_num = Cart.query.filter_by(customer_link=current_user.id).all() if current_user.is_authenticated else [], order=order)

@views.route('/products')
def products():
    products = Product.query.filter(Product.stock>0).all()
    
    return render_template("product.html", user=current_user, cart_num = Cart.query.filter_by(customer_link=current_user.id).all() if current_user.is_authenticated else [], products=products)
...
def cart():
    ...
    return render_template("cart.html", user=current_user, cart_num = Cart.query.filter_by(customer_link=current_user.id).all() if current_user.is_authenticated else [], cart=cart, amount=amount, total=amount+200, order=order)
...
def orders():
    ...
    return render_template('order.html', user=current_user, order=order, cart_num = Cart.query.filter_by(customer_link=current_user.id).all() if current_user.is_authenticated else [])
...
def search():
    if request.method == 'POST':
        ...
        return render_template('search.html', items=items, cart_num = Cart.query.filter_by(customer_link=current_user.id).all() if current_user.is_authenticated else [], user=current_user)
...
def view_product(product_id):
    ...
    return render_template('view-product.html', user=current_user, cart_num = Cart.query.filter_by(customer_link=current_user.id).all() if current_user.is_authenticated else [], product=product)
